    ...
    <div className="container-principal">
      <div className="department-page">
        ...
        <div className="department-card table-card">
          <div className="table-title"><h2>Emprunts en cours</h2><span>{emprunts.length}</span></div>
          <div className="table-scroll">
            <table>
              <thead><tr><th>Ressource</th><th>Exemplaire</th><th>Emprunteur</th><th>Type</th><th>Date emprunt</th><th>Retour prévu</th><th>Statut</th><th>Actions</th></tr></thead>
              <tbody>
                {pageActuelle.map((e) => (
                  <tr className="row-link" key={e.id}>
                    ...
                    <td>{TYPES_ADHERENT.find((t) => t.value === e.type_adherent)?.label}</td>
                    <td>{new Date(e.date_emprunt).toLocaleDateString('fr-FR')}</td>
                    <td>{new Date(e.date_retour_prevue).toLocaleDateString('fr-FR')}</td>
                    <td>
                      <span className={`badge ${e.est_en_retard ? 'badge-danger' : 'badge-blue'}`}>
                        <span className="dot"></span>{e.est_en_retard ? 'En retard' : 'En cours'}
                      </span>
                    </td>
                    <td>
                      ...
                      <button className="table-btn" onClick={() => navigate(`/bibliotheque/emprunts/retours?exemplaire=${e.exemplaire}`)}><i className="fas fa-undo"></i></button>
                    </td>
                  </tr>
                ))}
