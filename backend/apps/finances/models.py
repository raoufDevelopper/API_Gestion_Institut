...
class TarifQuerySet(models.QuerySet):
    ...
    def resoudre(self, type_paiement, specialite, niveau, annee_academique):
        ...
        candidats = self.filter(type_paiement=type_paiement, annee_academique=annee_academique, actif=True,)
...
class Tarif(models.Model):
    ...
    specialites = models.ManyToManyField("academique.Specialite", blank=True, related_name="tarifs", help_text="Laisser vide = s'applique à toutes les spécialités.",)

    niveaux = models.ManyToManyField("academique.Niveau", blank=True, related_name="tarifs", help_text="Laisser vide = s'applique à tous les niveaux.",)

    annee_academique = models.ForeignKey("academique.AnneeAcademique", on_delete=models.CASCADE, related_name="tarifs", null=True, blank=True)
...
class Paiement(TimeStampedModel):
    ...
    caisse_session = models.ForeignKey("CaisseSession", on_delete=models.PROTECT, null=True, blank=True, related_name="paiements",)
...
class Depense(TimeStampedModel):
    ...
    caisse_session = models.ForeignKey("CaisseSession", on_delete=models.PROTECT, null=True, blank=True, related_name="depenses",)

    justificatif = models.FileField(upload_to="depenses/justificatifs/%Y/%m/", storage=RawMediaCloudinaryStorage(), blank=True, null=True)

    statut = models.CharField(max_length=10, choices=Statut.choices, default=Statut.EN_ATTENTE)

    demande_par = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, null=True, related_name="depenses_demandees",)

    approuve_par = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, null=True, blank=True, related_name="depenses_approuvees",)
...
class Bourse(models.Model):
    ...
    etudiant = models.OneToOneField('utilisateurs.Etudiant', on_delete=models.CASCADE, related_name='bourse')
    ...
    type_reduction = models.CharField(max_length=15, choices=TYPE_REDUCTION_CHOICES, blank=True, null=True, help_text="Laisser vide pour utiliser le réglage par défaut de l'institut.")

    valeur_reduction = models.DecimalField(max_digits=10, decimal_places=2, blank=True, null=True, help_text="Laisser vide pour utiliser le réglage par défaut de l'institut.")
