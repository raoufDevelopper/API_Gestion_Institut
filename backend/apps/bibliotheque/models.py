...
class Ressource(models.Model):
    ...
    fichier_numerique = models.FileField(upload_to='bibliotheque/numerique/', storage=RawMediaCloudinaryStorage(), blank=True, null=True)
...
class Adherent(models.Model):
    ...
    etudiant = models.OneToOneField('utilisateurs.Etudiant', on_delete=models.CASCADE, null=True, blank=True, related_name='adherent_bibliotheque')

    personnel = models.OneToOneField('utilisateurs.Personnel', on_delete=models.CASCADE, null=True, blank=True, related_name='adherent_bibliotheque')

    formateur = models.OneToOneField('utilisateurs.Formateur', on_delete=models.CASCADE, null=True, blank=True, related_name='adherent_bibliotheque')
...
class Emprunt(models.Model):
    ...
    enregistre_par = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, null=True, blank=True, related_name='emprunts_enregistres')
...
def traiter_retour_exemplaire(exemplaire):
    ...
    reservation = Reservation.objects.filter(ressource=exemplaire.ressource, statut=StatutReservation.EN_ATTENTE).order_by('date_reservation').first()
    ...
    if reservation:
        ...
        user = _user_de_adherent(reservation.adherent)


        if user:
            creer_notification(
                destinataire=user, titre="Ouvrage disponible",
                message=f"« {exemplaire.ressource.titre} » est maintenant disponible. Retirez-le avant le {reservation.date_expiration}.",
...
def _user_de_adherent(adherent):
    """Corrige le bug signalé : Formateur n'a pas de .user direct, il passe par .personnel.user"""

    personne = adherent.personne

    if personne is None:
        return None

    return getattr(personne, 'user', None) or getattr(getattr(personne, 'personnel', None), 'user', None)
...
class Inventaire(models.Model):
    ...
    responsable = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, null=True, blank=True, related_name='inventaires')
...
class Acquisition(models.Model):
    ...
    enregistre_par = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, null=True, blank=True, related_name='acquisitions_enregistrees')
